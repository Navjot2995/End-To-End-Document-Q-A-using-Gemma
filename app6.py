...
if selected_theme == "Light":
    st.markdown("""
    <style>.main { background-color: #f4f4f5; color: #111827; }</style>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
    <style>.main { background-color: #0f172a; color: #f1f5f9; }</style>
    """, unsafe_allow_html=True)
...
uploaded_files = st.sidebar.file_uploader("Choose your PDFs", type=["pdf"], accept_multiple_files=True)
...
if st.button("💡 Send") and user_question:
    if st.session_state.vectors is None:
        ...
        with st.spinner("Gemma is generating a response..."):
            ...
            st.session_state.chat_history.append((user_question, response['answer'], datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
